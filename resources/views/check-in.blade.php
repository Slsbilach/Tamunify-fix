    ...
    <div class="flex min-h-screen flex-col relative overflow-hidden">
        ...
        <header class="bg-white/90 border-b relative z-10">
            <div class="w-full flex justify-between items-center">
                {{-- Tempatkan logo/navigasi jika diperlukan --}}
            </div>
        </header>
        ...
        <main class="flex-1 relative z-10 px-4 py-10 md:py-14">
            <div class="max-w-6xl mx-auto space-y-16">
                ...
                <div class="grid md:grid-cols-3 gap-8">
                    ...
                    <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                        @include('components.card-default', [
                            ...
                            'title' => 'Magang/PKL',
                            'desc' => 'Untuk mahasiswa atau siswa yang akan melaksanakan magang atau praktik kerja lapangan.',
                            'href' => '/register/internship'
                        ])
                        @include('components.card-default', [
                            'icon' => '<i class="fa-solid fa-clock w-8 h-8 text-green-700"></i>',
                            'title' => 'Tamu Berulang',
                            'desc' => 'Untuk orang tua/wali, vendor, atau tamu yang perlu akses berulang dalam periode tertentu.',
                            'href' => '/register/recurring'
                        ])
                    </div>
                </div>

                {{-- Cara Kerja Sistem --}}
                <section class="bg-white p-8 rounded-2xl shadow-xl max-w-5xl mx-auto text-center border border-gray-100 relative overflow-hidden">
                    <div class="absolute inset-0 bg-gradient-to-br from-green-50 to-transparent opacity-50 pointer-events-none"></div>
                    <h3 class="text-2xl font-bold mb-8 text-green-900 relative z-10">Cara Kerja Sistem</h3>

                    {{-- Desktop View --}}
                    <div class="hidden md:grid grid-cols-[1fr_40px_1fr_40px_1fr_40px_1fr] gap-0 relative z-10 items-center">
                    ...
                    <div class="md:hidden flex flex-col items-center gap-8 relative z-10">
                        @foreach ($steps as $index => $item)
                            <div class="text-center">
                                ...
                                <p class="text-gray-700 text-sm">{{ $item['desc'] }}</p>
                                @if ($index < count($steps) - 1)
                                    <i data-lucide="arrow-right" class="w-5 h-5 text-gray-400 mt-4 mx-auto"></i>
                                @endif
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="flex justify-center">
                    <a href="/" class="inline-flex items-center gap-2 text-white font-medium bg-green-800 hover:bg-green-900 px-5 py-3 rounded-lg transition-all duration-300 hover:scale-105 shadow-md hover:shadow-lg">
